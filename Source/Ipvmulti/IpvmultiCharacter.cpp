...
void AIpvmultiCharacter::HandleFire_Implementation()
{
	if (!CanFire())
	{
		...
		{
			GEngine->AddOnScreenDebugMessage(-1, 2.f, FColor::Red, TEXT("No ammo!"));
		}
		return;
	}
...
void AIpvmultiCharacter::OnRep_Ammo_Implementation()
{
	UE_LOG(LogTemp, Log, TEXT("Ammo updated: %d"), CurrentAmmo);
}
